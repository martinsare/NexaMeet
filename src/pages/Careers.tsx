    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="relative mx-auto max-w-4xl px-6 py-24 text-center">
          <FadeIn>
            <p className="mb-4 text-xs font-semibold uppercase tracking-widest text-text-muted">Join the team</p>
            <h1 className="font-display text-4xl font-semibold text-text md:text-5xl">
              Build the future of <span className="">remote work.</span>
            </h1>
            ...
            <div className="mt-4 flex items-center justify-center gap-6 text-sm text-text-muted">
              <span className="flex items-center gap-1.5"><Zap className="h-3.5 w-3.5 text-primary" /> 28 people</span>
              <span className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5 text-success" /> 12 countries</span>
              <span className="flex items-center gap-1.5"><Clock className="h-3.5 w-3.5 text-destructive" /> Series A</span>
            </div>
          </FadeIn>
        </div>
      </section>
      ...
      {/* Perks */}
      <section className="border-b border-border bg-surface-raised/60 py-20">
        <div className="mx-auto max-w-5xl px-6">
          <FadeIn>
            <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-text-muted">Why NexaMeet</p>
            <h2 className="font-display text-2xl font-semibold text-text md:text-3xl">Perks & benefits</h2>
          </FadeIn>
            ...
            {PERKS.map((perk, i) => (
              <FadeIn key={perk.title} delay={i * 0.07}>
                <div className="rounded-2xl border border-border bg-surface-card p-6">
                  ...
                  <h3 className="mt-3 font-semibold text-text">{perk.title}</h3>
                  <p className="mt-1.5 text-sm text-text-muted leading-relaxed">{perk.body}</p>
                </div>
              </FadeIn>
            ))}
          </div>
      ...
      <section className="mx-auto max-w-5xl px-6 py-24">
        <FadeIn>
          <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-text-muted">Now hiring</p>
          <h2 className="font-display text-2xl font-semibold text-text md:text-3xl">Open roles</h2>
        </FadeIn>
              ...
              <div className="group flex flex-col gap-4 py-8 md:flex-row md:items-center md:justify-between">
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-medium text-primary">{role.team}</span>
                  </div>
        ...
        <FadeIn>
          <p className="mt-8 text-sm text-text-muted">
            ...
            <a href="mailto:jobs@nexameet.dev" className="text-primary hover:text-text transition-colors underline underline-offset-4">
              jobs@nexameet.dev
            </a>
